...
// Returns the number of characters written to `out` (at most 6).
inline size_t decodeReport(const uint8_t* report, uint8_t* prevKeys, char* out, size_t outCap) {
  ...
  for (size_t i = 0; i < 6; ++i) {
    ...
    if (written < outCap) out[written++] = c;
  }
