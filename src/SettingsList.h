...
inline std::vector<SettingInfo> getSettingsList(const SdCardFontRegistry* registry = nullptr) {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    // the whole list was materialised as one stack temporary (45 x sizeof(SettingInfo)
    // = 5,580 bytes), which made this frame 6,240 bytes measured with -fstack-usage on
    // the riscv32 target — the largest non-vendor frame in the firmware, 24x the
    // 256-byte Resource Protocol limit. push_back() keeps only one 124-byte SettingInfo
    // on the stack at a time; the entries, their order and their values are unchanged.
    //
    // Exact final capacity: the 45 fixed entries below plus the one conditional tilt
    // entry appended further down (X3 only), so neither the push_backs nor that
    // insert() reallocate. Keep in sync when adding an entry (Resource Protocol 7).
    constexpr size_t FIXED_ENTRY_COUNT = 45;
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_SLEEP_SCREEN, &CrossPointSettings::sleepScreen,
                                  {StrId::STR_DARK, StrId::STR_LIGHT, StrId::STR_CUSTOM, StrId::STR_COVER,
                                   ...
                                   StrId::STR_SLEEP_CALENDAR}, "sleepScreen", StrId::STR_CAT_DISPLAY));
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_REFRESH_FREQ, &CrossPointSettings::refreshFrequency,
                                  {StrId::STR_PAGES_1, StrId::STR_PAGES_5, StrId::STR_PAGES_10, StrId::STR_PAGES_15,
                                   StrId::STR_PAGES_30}, "refreshFrequency", StrId::STR_CAT_DISPLAY));
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_UI_THEME, &CrossPointSettings::uiTheme,
                                  {StrId::STR_THEME_CLASSIC, StrId::STR_THEME_LYRA, StrId::STR_THEME_LYRA_EXTENDED,
                                   StrId::STR_THEME_ROUNDEDRAFF, StrId::STR_THEME_LYRA_SIX}, "uiTheme",
                                  StrId::STR_CAT_DISPLAY));
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_PARA_ALIGNMENT, &CrossPointSettings::paragraphAlignment,
                                  {StrId::STR_JUSTIFY, StrId::STR_ALIGN_LEFT, StrId::STR_CENTER, StrId::STR_ALIGN_RIGHT,
                                   StrId::STR_BOOK_S_STYLE}, "paragraphAlignment", StrId::STR_CAT_READER));
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_IMAGES, &CrossPointSettings::imageRendering,
                                  {StrId::STR_IMAGES_DISPLAY, StrId::STR_IMAGES_PLACEHOLDER,
                                   StrId::STR_IMAGES_SUPPRESS}, "imageRendering", StrId::STR_CAT_READER));
    // --- Controls ---
    ...
    v.push_back(SettingInfo::Toggle(StrId::STR_FRONT_BTN_FOLLOW_ORIENTATION,
                                    ...
                                    StrId::STR_CAT_CONTROLS));
    // Both of these dropped their trailing choice: ORIENTATION_CHANGE
    // long-press-rotated the screen, and LP_MENU_BOOKMARK long-pressed to
    // toggle a bookmark. Each was the LAST enum value, so indices 0 and 1
    // are unchanged. longPressMenuFunction still remaps its retired index on
    // load (normalizeRetiredSettings()); longPressButtonBehavior no longer
    // does, because index 2 has been REUSED for FONT_SIZE_STEP below.
    //
    // Third label = stored value 2 = FONT_SIZE_STEP. The order of this array IS
    // the persisted encoding, so append only — see LONG_PRESS_BUTTON_BEHAVIOR
    // in CrossPointSettings.h for why the retired slot was reused rather than a
    // fourth value added.
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_LONG_PRESS_MENU, &CrossPointSettings::longPressMenuFunction,
                                  {StrId::STR_KOSYNC, StrId::STR_DISABLED}, "longPressMenuFunction",
                                  StrId::STR_CAT_CONTROLS));
    v.push_back(SettingInfo::Enum(StrId::STR_SHORT_PWR_BTN, &CrossPointSettings::shortPwrBtn,
                                  {StrId::STR_IGNORE, StrId::STR_SLEEP, StrId::STR_PAGE_TURN, StrId::STR_FORCE_REFRESH,
                                   StrId::STR_FOOTNOTES}, "shortPwrBtn", StrId::STR_CAT_CONTROLS));
    ...
    v.push_back(SettingInfo::Value(StrId::STR_TIME_TO_SLEEP, &CrossPointSettings::sleepTimeoutMinutes,
                                   {CrossPointSettings::MIN_SLEEP_TIMEOUT_MINUTES,
                                    CrossPointSettings::MAX_SLEEP_TIMEOUT_MINUTES, 1}, "sleepTimeoutMinutes",
    ...
    v.push_back(SettingInfo::Enum(StrId::STR_PROGRESS_BAR_THICKNESS, &CrossPointSettings::statusBarProgressBarThickness,
                                  {StrId::STR_PROGRESS_BAR_THIN, StrId::STR_PROGRESS_BAR_MEDIUM,
                                   StrId::STR_PROGRESS_BAR_THICK}, "statusBarProgressBarThickness",
                                  StrId::STR_CUSTOMISE_STATUS_BAR));
