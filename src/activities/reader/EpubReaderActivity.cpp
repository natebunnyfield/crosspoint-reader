...
void EpubReaderActivity::loop() {
  ...
  // following the hold does not also open the menu; so does a line-spacing chord.
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
...
void EpubReaderActivity::stepReaderFontSize(const int delta) {
  const uint8_t current = SETTINGS.fontSize;
  const uint8_t next = ReaderUtils::steppedFontSize(current, delta);
...

  {
    ...
    // Guarded by the next == current early-return above, so this only ever runs on a real
