...
CalendarFonts resolveFonts(GfxRenderer& renderer) {
  CalendarFonts f{};
  // EXTRA_LARGE maps to the largest installed size (18pt in the standard pack).
  const int sdId = sdFontSystem.loadForDisplay(
      PREFERRED_SD_FAMILY, CrossPointSettings::FONT_SIZE::EXTRA_LARGE, renderer);
  if (sdId != 0) {
    f.display = sdId;
  } else {
