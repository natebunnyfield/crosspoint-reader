...
// cannot select a value the firmware supports, which is far worse.
inline std::vector<uint8_t> resolve(const std::vector<uint8_t>& order, size_t count) {
  if (isPermutation(order, count)) return order;
...
inline size_t positionOf(const std::vector<uint8_t>& order, size_t count, uint8_t stored) {
  const std::vector<uint8_t> resolved = resolve(order, count);
...
template <typename T>
std::vector<T> reorder(const std::vector<uint8_t>& order, const std::vector<T>& values) {
  const std::vector<uint8_t> resolved = resolve(order, values.size());
  std::vector<T> out;
