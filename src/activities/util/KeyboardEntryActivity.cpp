...
// ---------------------------------------------------------------------------
// Hi-res backspace key icon. The SDK's keyboard() draws the delete key as its
// 16px Lucide "delete" mask stretched onto the LOGICAL pixel grid, so on a
// supersampled host build it stays chunky next to the crisp 2x UI text (none
// of the four System font families cover a backspace codepoint -- U+232B,
// U+2326, U+2190 and U+2715 all miss in the built-in interval tables -- so a
// text glyph through the hi-res font path is not an option). This is the same
// art rasterised from the SDK's delete.svg at 64px through gen_icons.py's
// pipeline (threshold 110, Mask1: MSB-first, 1 = transparent, 0 = ink; the
// rasteriser reproduces lucideDeleteIcon16()'s bytes exactly at 16px), blitted
// per DEVICE pixel like a hi-res font companion. Flash-resident; compiled out
// entirely at scale 1, so device builds are untouched.
static constexpr int BACKSPACE_ICON_2X_PX = 64;
static constexpr uint8_t BACKSPACE_ICON_2X_BITS[] = {
...
static_assert(sizeof(BACKSPACE_ICON_2X_BITS) == BACKSPACE_ICON_2X_PX * BACKSPACE_ICON_2X_PX / 8,
              "backspace icon size mismatch");

// DrawTarget wrapper for the keyboard render pass. Forwards everything to the
// stock GfxRendererTarget (which is final, hence composition) except the one
// bitmap the keyboard component ever draws -- the delete key's 16px mask,
// recognised by its data pointer (lucideDeleteIcon16() is an inline function,
// so its static array is one object program-wide) -- which is redrawn from the
// 64px art above straight onto the device pixel grid via drawPixelDevice().
// Same logical rect, same Contain semantics, so layout and hit rects are
// untouched; only the sampling grid is finer.
class HiResKeyboardTarget final : public fui::DrawTarget {
 public:
  explicit HiResKeyboardTarget(const GfxRenderer& renderer) : inner(renderer), renderer(renderer) {}

  void setFont(const fui::FontId slot, const int gfxFontId) { inner.setFont(slot, gfxFontId); }
  fui::DeviceContext deviceContext() const { return inner.deviceContext(); }
  ...
  int16_t lineHeight(const fui::FontId font) const override { return inner.lineHeight(font); }
  ...
  void bitmap(const fui::Rect rect, const fui::BitmapRef bitmap, const fui::BitmapMode mode,
    ...
    if (bitmap.data == fui::lucideDeleteIcon16().data && !rect.empty()) {
      drawHiResBackspace(rect, foreground);
      return;
    }
 ...
 private:
  fui::GfxRendererTarget inner;
  const GfxRenderer& renderer;

  void drawHiResBackspace(const fui::Rect rect, const fui::Paint foreground) const {
    constexpr int S = GfxRenderer::RENDER_SCALE;
    constexpr int src = BACKSPACE_ICON_2X_PX;
    ...
    for (int py = 0; py < box; ++py) {
      ...
      for (int px = 0; px < box; ++px) {
        ...
        const bool ink = ((BACKSPACE_ICON_2X_BITS[sy * rowBytes + (sx >> 3)] >> (7 - (sx & 7))) & 1) == 0;
...
void KeyboardEntryActivity::render(RenderLock&&) {
  ...
  interactionsReady = false;
#if CROSSPOINT_RENDER_SCALE > 1
  // Wrapper redraws the delete key's icon on the device pixel grid; identical
  // to the stock target in every other respect (see HiResKeyboardTarget).
  HiResKeyboardTarget target(renderer);
#else
