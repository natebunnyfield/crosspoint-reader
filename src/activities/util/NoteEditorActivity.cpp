...
void NoteEditorActivity::render(RenderLock&&) {
  ...
  if (caretMode) {
    // The band the byte count normally occupies is the only line of prose on
    // this screen, so the mode borrows it rather than adding a sixth band. The
    // string is the full-screen keyboard's own cursor-mode hint, reused
    // verbatim — same gesture, same words, already translated.
    renderer.drawText(SMALL_FONT_ID, metrics.contentSidePadding, statusY, tr(STR_KB_HINT_MOVE_CURSOR));
  } else {
    char status[96];
