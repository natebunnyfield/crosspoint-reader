...
3. **XML encoding tables** (generated by `tools/gen_xml_encodings.py`):
   - `lib/Epub/Epub/parsers/XmlEncodingTables.cpp`
   - **Source**: Python's own codec tables, via the encoding list at the top of
     the generator. Committed, unlike the two above: it changes only when a code
     page is added, and building it needs a Python with the codecs installed.
   - **To modify**: edit the `ENCODINGS` list, run `python3
     tools/gen_xml_encodings.py`. Never hand-edit the `.cpp` — the generator
     refuses a code page expat could not accept (a low half that is not ASCII,
     a value outside the BMP), and a hand edit skips that check.
   - Why it exists: expat builds in four encodings and refuses every other
     declaration, so an EPUB in windows-1252 could not be opened at all.
     `docs/encodings-glyphs-and-library-sync-2026-08-23.md`.
