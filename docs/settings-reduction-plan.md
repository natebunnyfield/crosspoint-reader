...
|---|---|---|---|---|
| Reader Font (font family + size) | LibreFranklin / 14pt | **KEEP** | — | The product. You curate .cpfont families weekly. |
...
| `ligatures` + `ligaturesOff` — **Typography Settings** | 2026-08-24 | on / nothing suppressed | Owner ruling: *"give a full subpage of Typography Settings that gives all available typography options with full granularity, including toggling each individual ligature"*, from *"almendra has at least one distracting ligature (st)"*. Not hardcodable in either direction: it is a reading-taste preference whose very ask was granularity. Both rows are `STR_CAT_READER`, so they persist and serve the web API while the DEVICE edits them on their own screen — the per-pair rows are one per pair the loaded family carries (Edgar fourteen, Almendra four), which no static list can express. The suppression set is stored as a string keyed on the INPUT pair, never the output codepoint; `docs/ligature-control.md` has the whole argument and the Almendra `fh`→U+FB00 evidence for it. Reaches `ReaderRenderSpec` as a fingerprint, because a ligature changes an advance and so moves line breaks. |
| `lineSpacing` — **Line Spacing**, row REINSTATED | 2026-08-24 | NORMAL | Owner ruling: *"put or move line grid, line spacing, letter spacing, justified text to Typography Settings."* This row was DELETED by the 2026-08-21 reduction in this very document; the ruling above supersedes that **for this field only**, and the field itself never went — it is the one reading-taste entry that stayed non-`constexpr`, because the reader has a designed chord (Confirm held + a side button) that steps it. The row lives on Typography Settings; `STR_CAT_READER`, `Enum` over `LINE_COMPRESSION` with labels indexed BY ENUM VALUE. Its hand-written `toJson`/`fromJson` pair is retired with the row's return — a `valuePtr` row is carried by the generic loop, and two writers of one key is what that pair worked around. It also reaches the web settings API for the first time. |
| Line Grid, Justified Text — **MOVED, not added** | 2026-08-24 | unchanged | Same ruling. Both were device-visible `STR_CAT_SYSTEM` rows and are now `STR_CAT_READER`, which `rebuildSettingsLists()` drops from the flat Settings list; the Typography screen SELECTS them out of `getSettingsList()` by name. Nothing about either row's storage, key, labels or web-API presence changes — the move is one word each. **Letter spacing, the fourth name in that ruling, was NOT built**: no `letterSpacing` field and no tracking in the layout engine, so it is a new feature (per-glyph advance, re-pagination, a cache bump), not a move, and is being scoped separately. |
